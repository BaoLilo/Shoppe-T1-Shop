...
function logoutUser(redirectTo){
  ...
  setTimeout(()=> window.location.href = redirectTo || "index2.html", 900);
}
...
function requireLogin(currentPage){
  ...
  window.location.href = "login2.html?redirect=" + encodeURIComponent(currentPage);
  return true;
}

/* Update the header's account link: shows the user's name + points to
   the orders page when logged in, otherwise points to login2.html */
...
function highlightActiveNav(){
  const current = window.location.pathname.split("/").pop() || "index2.html";
